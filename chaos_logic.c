...
//                     if (stars[index].move_on_x == 0) stars[index].move_on_x = 1;
//                     stars[index].move_on_y = (rand() % 3) - 1;
//                     if (stars[index].move_on_y == 0) stars[index].move_on_y = 1;
//                     stars[index].type = ORDER;
//                     stars[index].score = 0;
//                     stars[index].is_active = 1;
//                 }
...
//         star2->move_on_y = temp_y;
//     }
//     // CHAOS + ORDER = CHAOS ест ORDER (CHAOS НЕ ЕСТ CHAOS!)
//     else if ((star1->type == CHAOS && star2->type == ORDER) ||
//              (star2->type == CHAOS && star1->type == ORDER))
//     {
//         astra *chaos_star = (star1->type == CHAOS) ? star1 : star2;
...
//         }
//     }
//     // CHAOS + CHAOS - просто отскок (НЕ УНИЧТОЖАЮТСЯ!)
//     else if (star1->type == CHAOS && star2->type == CHAOS)
//     {
//         // Меняем направления
//         int temp_x = star1->move_on_x;
...
//         star2->move_on_y = temp_y;
//     }
//     // ALFA_CHAOS ест ORDER (НЕ ЕСТ CHAOS!)
//     else if ((star1->type == ALFA_CHAOS && star2->type == ORDER) ||
//              (star2->type == ALFA_CHAOS && star1->type == ORDER))
//     {
//         astra *alfa_chaos = (star1->type == ALFA_CHAOS) ? star1 : star2;
//         astra *order_star = (star1->type == ALFA_CHAOS) ? star2 : star1;
        
//         alfa_chaos->score++;
//         order_star->is_active = 0;
        
//         // ALFA_CHAOS после 50 съеденных ORDER становится MEGA_CHAOS
//         if (alfa_chaos->score >= 50 && alfa_chaos->type == ALFA_CHAOS) {
//             alfa_chaos->type = MEGA_CHAOS;
//             alfa_chaos->score = 0;
//         }
//     }
//     // MEGA_CHAOS ест всех кроме ALFA_ORDER (включая CHAOS)
//     else if ((star1->type == MEGA_CHAOS && star2->type != ALFA_ORDER) ||
//              (star2->type == MEGA_CHAOS && star1->type != ALFA_ORDER))
//     {
//         astra *mega_chaos = (star1->type == MEGA_CHAOS) ? star1 : star2;
...
//         }
//     }
//     // ORDER и CHAOS (оставшиеся случаи) - ничего не делаем
//     // (все случаи уже обработаны выше)
// }
...
void handle_collision(astra *star1, astra *star2, astra stars[], int max_count)
{
    ...
    // MEGA_CHAOS ест всех КРОМЕ MEGA_CHAOS и ALFA_ORDER
    else if ((star1->type == MEGA_CHAOS && star2->type != ALFA_ORDER && star2->type != MEGA_CHAOS) ||
             (star2->type == MEGA_CHAOS && star1->type != ALFA_ORDER && star1->type != MEGA_CHAOS))
    {
        astra *mega_chaos = (star1->type == MEGA_CHAOS) ? star1 : star2;
        astra *other = (star1->type == MEGA_CHAOS) ? star2 : star1;
        
        // other точно не MEGA_CHAOS и не ALFA_ORDER
        mega_chaos->score++;
        other->is_active = 0;
    }
    // MEGA_CHAOS + MEGA_CHAOS = просто отскок (не уничтожаются)
    else if (star1->type == MEGA_CHAOS && star2->type == MEGA_CHAOS)
    {
        // Меняем направления
        int temp_x = star1->move_on_x;
        int temp_y = star1->move_on_y;
